  ...
  if (m->scrub_pgs.empty()) {
    for (hash_map<pg_t, PG*>::iterator p = pg_map.begin();
	 ...
	 p++) {
      ...
      if (pg->is_primary()) {
	pg->unreg_scrub();
	pg->state_set(PG_STATE_FORCESCRUB);
	if (m->repair)
	  pg->state_set(PG_STATE_FORCEREPAIR);
	else
	  pg->state_clear(PG_STATE_FORCEREPAIR);
	pg->reg_scrub();
...
void OSD::sched_scrub()
{
  ...
  while (p != last_scrub_pg.end()) {
    ...
    if (pg) {
      if (pg->is_active() &&
	  (should ||
	   pg->state_test(PG_STATE_FORCESCRUB) ||
	   pg->state_test(PG_STATE_FORCEREPAIR)) &&
	  !pg->sched_scrub()) {
	pg->unlock();
	sched_scrub_lock.Lock();
	break;
